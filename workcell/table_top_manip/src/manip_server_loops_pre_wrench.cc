...
// NOTE: this version has NO wrench/haptic force feedback integration.
void ManipServer::teleop_loop(const RUT::TimePoint& time0) {
  ...
  // Integration position for amplify mode
  Eigen::Vector3d robot_amp_pos = Eigen::Vector3d::Zero();
  ...
  while (true) {
    ...
    bool teleop_active_local;
    {
      std::lock_guard<std::mutex> lock(_ctrl_mtx);
      teleop_active_local = _teleop_active;
    }

    // ── Idle: wait for Button 1 rising edge to start teleop ──────────────
    if (!teleop_active_local) {
      if (btn1 && !was_btn1) {
        ref_x = state.position[0] / 1000.0;
        ref_y = state.position[1] / 1000.0;
        ref_z = state.position[2] / 1000.0;
        // JY: capture gimbal reference instead of quat reference
        ...
        start_saving_data_for_a_new_episode();  // JY
        std::cout << header << "Teleop activated. Recording started.\n";
      }
      was_btn1 = btn1;
      was_btn2 = btn2;
      usleep(5000);  // 200 Hz
    ...
    if (btn1 && btn2) {
      ...
      stop_saving_data();  // JY
      std::cout << header << "Teleop deactivated. Recording stopped.\n";

      // Wait for both buttons to be fully released before returning to idle
      // so the next Button 1 rising edge is not triggered immediately.
    ...
    // ── Button 2 rising edge -> enter amplify mode ────────────────────────
    if (btn2 && !was_btn2) {
      if (!_config.mock_hardware)
        robot_ptrs[0]->getCartesian(robot_initial_pose);
      robot_amp_pos = robot_initial_pose.head<3>();
      amplify_mode = true;
      std::cout << header << "Amplify mode ON.\n";
    }
    // ── Button 2 falling edge -> exit amplify, re-reference ───────────────
    else if (!btn2 && was_btn2 && amplify_mode) {
      if (!_config.mock_hardware)
        robot_ptrs[0]->getCartesian(robot_initial_pose);
      ref_x = state.position[0] / 1000.0;
      ref_y = state.position[1] / 1000.0;
      ref_z = state.position[2] / 1000.0;
      // JY: re-reference gimbal angles instead of quat on amplify exit
      ref_gimbals[0] = state.gimbal_angles[0];
      ref_gimbals[1] = state.gimbal_angles[1];
      ref_gimbals[2] = state.gimbal_angles[2];
      q_robot_initial = Eigen::Quaterniond(
    ...
    const double orientation_scale = 0.6;
    double j_rel[3] = {
    ...
    if (amplify_mode) {
      // Velocity integration: Touch velocity -> scaled -> robot frame -> integrate
      const double dt = 0.005;
      Eigen::Vector3d vel_m_s(state.velocity[0] * 1e-3,
                              state.velocity[1] * 1e-3,
                              state.velocity[2] * 1e-3);
      Eigen::Vector3d v_cmd = hd2rob_R * vel_m_s * _config.teleop_vel_scale;
      for (int i = 0; i < 3; i++)
        v_cmd[i] = std::clamp(v_cmd[i],
                              -_config.teleop_vel_max, _config.teleop_vel_max);
      Eigen::Vector3d delta = v_cmd * dt;
      // Per-step safety clamp: max 3 mm per 5 ms (matches Python code)
      for (int i = 0; i < 3; i++)
        delta[i] = std::clamp(delta[i], -0.003, 0.003);
      robot_amp_pos += delta;
      target_pose.head<3>() = robot_amp_pos;
    } else {
      // Absolute delta-position mode
      double px = _touch_config.pos_scale * (state.position[0] / 1000.0 - ref_x);
      double py = _touch_config.pos_scale * (state.position[1] / 1000.0 - ref_y);
      double pz = _touch_config.pos_scale * (state.position[2] / 1000.0 - ref_z);
  ...
  stop_saving_data();  // JY: ensure recording is closed if program exits mid-session
